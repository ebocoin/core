...
#if defined(NDEBUG)
# error "Ebocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa494eeb17b803c0874e66055108dfd5cc24363bb5c2a47650b72a4c9c655f24e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ebocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ebocoin Signed Message:\n";
...
{
    // Ebocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 700800); // Ebocoin: 700800 blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 12 * 60 * 60; // ebocoin: 12 H
static const int64 nTargetSpacing = 1.5 * 60; // Ebocoin: 1.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ebocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ebocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "EBO coin launched";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c184717fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1531758233;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1495272;
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd0d929e05a53a4308ceec184e6a75233ecfa127fd3ed0180d3ca1f04cb6e4145"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xce, 0xd6, 0xd9 }; // Ebocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EbocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EbocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EbocoinMiner(CWallet *pwallet)
{
    printf("EbocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ebocoin-miner");
    ...
    try { loop {
        ...
        printf("Running EbocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EbocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EbocoinMiner, pwallet));
}
